...
static void Mock_Log(void* instance, PeripheralLogLevel level, const char* fmt, ...) {
    (void)instance; (void)level; (void)fmt;
}
...
static void Mock_RegisterIO(int slot, PeripheralIOHandler readC0, PeripheralIOHandler writeC0,
                             PeripheralIOHandler readCx, PeripheralIOHandler writeCx) {
    (void)slot; (void)readC0; (void)writeC0; (void)readCx; (void)writeCx;
}

static void Mock_RegisterCxROM(int slot, uint8_t* rom_ptr) {
    (void)slot; (void)rom_ptr;
}
...
TEST_CASE("Clock Peripheral: Time Latching Behavior") {
    ...
    // We can't easily predict the exact time, but we can verify they are valid BCD (0-9)
...
TEST_CASE("Clock Peripheral: State Persistence") {
    ...
    for (uint16_t i = 0; i < 10; ++i) {
        latched_values[i] = g_mock_handlers[0xC080 + i].read(instance1, 0, 0xC080 + i, 0, 0, 0);
    }
    ...
    for (uint16_t i = 0; i < 10; ++i) {
        CHECK(g_mock_handlers[0xC080 + i].read(instance2, 0, 0xC080 + i, 0, 0, 0) == 0);
    }

    // Load the saved state
    PeripheralStatus status = g_clock_peripheral.load_state(instance2, buffer.data(), state_size);
    CHECK(status == PERIPHERAL_OK);

    // 5. Verify restored values match original latched values
    for (uint16_t i = 0; i < 10; ++i) {
        uint8_t restored = g_mock_handlers[0xC080 + i].read(instance2, 0, 0xC080 + i, 0, 0, 0);
