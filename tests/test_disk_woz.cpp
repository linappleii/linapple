...
extern auto reconstruct_bitstream_nibble(const uint8_t* buffer, uint32_t bit_count,
...
TEST_CASE("DiskWOZ: [WOZ-3] All-zero bitstream does not infinite loop") {
  ...
  uint32_t bit_idx = 0;
  uint8_t nibble = reconstruct_bitstream_nibble(zeros.data(), zeros.size() * 8, &bit_idx);
  CHECK(nibble == 0);
}

TEST_CASE("DiskWOZ: [WOZ-1/2] Corrupted chunk size does not loop or crash open") {
  const char* temp_woz = "corrupted_chunk.woz";
  {
    ...
    hdr[12] = 'I'; hdr[13] = 'N'; hdr[14] = 'F'; hdr[15] = 'O';
    hdr[16] = 0xFF; hdr[17] = 0xFF; hdr[18] = 0xFF; hdr[19] = 0xFF; // chunk_size = 0xFFFFFFFF
