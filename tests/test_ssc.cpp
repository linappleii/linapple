...
void RegisterIoHandler(uint32_t slot, iofunction r, iofunction w, iofunction cr,
                       iofunction cw, void* p, uint8_t* rom) {
  (void)slot;
  (void)r;
  (void)w;
  (void)cr;
...
auto IO_Null(uint16_t nPC, uint16_t nAddr, uint8_t nWriteFlag,
             uint8_t nWriteValue, uint32_t nCyclesLeft) -> uint8_t {
  (void)nPC;
  (void)nAddr;
  (void)nWriteFlag;
  (void)nWriteValue;
