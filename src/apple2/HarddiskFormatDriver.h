...
typedef enum { HARDDISK_DRIVER_CAP_WRITE = 0x01 } HarddiskDriverCap_e;
...
/*
 * Register a harddisk driver by populating this struct and passing it to
 * HarddiskLoader. All function pointers must be non-NULL unless documented
 * otherwise.
 */
typedef struct HarddiskFormatDriver_t {
  ...
  /* Called after probe() succeeds. out_os_readonly is set to true if the file
   * was opened read-only by the OS. Writes an opaque instance to *out_instance.
   */
  HarddiskError_e (*open)(const char* path, bool* out_os_readonly,
                          void** out_instance);
