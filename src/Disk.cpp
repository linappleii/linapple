...
int DiskInsert(int drive, LPCTSTR imagefilename, BOOL writeprotected, BOOL createifnecessary)
{
  ...
  if (lf > 3 && imagefilename[lf - 1] == 'z' && imagefilename[lf - 2] == 'g' && imagefilename[lf - 3] == '.') {// .gz?
  ...
  int error = ImageOpen(/*imagefilename*/tmp, &fptr->imagehandle, &fptr->writeprotected, createifnecessary);
  if (error == IMAGE_ERROR_NONE) {// if no errors
