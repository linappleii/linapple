...
// Prototypes _______________________________________________________________
  static  int ParseInput ( char* pConsoleInput, bool bCook = true );
...
//    if (bUpdateTop)
    ...
    // g_nDisasmCurLine = 0;
      ...
      sprintf( sText, "DisasmCalcTopFromCurAddress(\n"
        "\tTop: %04X\n"
        "\tLen: %04X\n"
        "\tMissed: %04X",
...
Update_t CmdDisk ( int nArgs)
{
  ...
  else
  {
    ...
    const char* pDiskName = g_aArgs[ 3 ].sArg;
...
Update_t CmdOutputCalc (int nArgs)
{
  ...
  //    CHC_ARG_
  //    CHC_STRING
  snprintf( sText, sizeof(sText), "$%04X  0z%08X  %5d  '%c' ",
...
Update_t CmdOutputPrint (int nArgs)
{
  ...
  for (iArg = 1; iArg <= nArgs; iArg++ )
  {
    ...
    {
      int iChar;
      int nChar = strlen( g_aArgs[ iArg ].sArg );
      ...
      for( iChar = 0; iChar < nChar; iChar++ )
      {
        ...
        switch ( eThis )
        {
          ...
          case PS_TYPE:
            ...
            {
              ConsoleBufferPushFormat( sText, "Error: Missing value arg: %d", iValue + 1 );
              return ConsoleUpdate();
            }
            switch( c )
            {
...
void DebugContinueStepping(const bool bCallerWillUpdateDisplay/*=false*/)
{
  ...
  if (g_nDebugSteps)
  {
    ...
    if (regs.pc == g_nDebugStepUntil || g_bDebugBreakpointHit)
    {
      ...
      ConsoleBufferPushFormat( sText, "Stop reason: %s", pszStopReason );
      ConsoleUpdate();
