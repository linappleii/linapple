...
auto GetSerialPortPath() -> std::string& {
  static std::string s_sSerialPortPath = "";
  return s_sSerialPortPath;
}
      ...
      if (n > 0) {
        ...
        for (int i = 0; i < n; ++i) {
          Peripheral_Command(2, SSC_CMD_PUSH_RX_BYTE,
                             &buffer.at(static_cast<size_t>(i)), 1);
        }
        pthread_mutex_unlock(&g_CriticalSection);
      }
    }
    constexpr useconds_t POLL_DELAY = 1000;
    usleep(POLL_DELAY);  // Poll every 1ms
  }
  return nullptr;
}
...
auto SSCFrontend_IsActive() -> bool {
  ...
  if ((g_hCommHandle == -1) && !GetSerialPortPath().empty()) {
    g_hCommHandle =
        open(GetSerialPortPath().c_str(), O_RDWR | O_NOCTTY | O_NDELAY);
    if (g_hCommHandle != -1) {
...
void SSCFrontend_Update(SuperSerialCard* pSSC, uint32_t totalcycles) {
  ...
  g_dwCommInactivity += totalcycles;
  constexpr uint32_t COMM_IDLE_LIMIT = 1000000;
  if (g_dwCommInactivity > COMM_IDLE_LIMIT) {
