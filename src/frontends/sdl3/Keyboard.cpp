...
static constexpr uint8_t kA2Bs       = 0x08; // Backspace and Left arrow both send this
...
static const Apple2KeyboardMap_t* g_current_locale_map = &Map_US;
// NOLINTNEXTLINE(cppcoreguidelines-avoid-non-const-global-variables)
static int g_keyboard_mapping_mode = 0; // 0=Symbolic, 1=Positional
...
void Frontend_UpdateKeyboardMapping() {
    ...
    if (ConfigLoadString("Keyboard", "Layout", &locale)) {
        if (locale == "French") {
            g_current_locale_map = &Map_FR;
        } else if (locale == "German") {
            g_current_locale_map = &Map_DE;
        } else {
            g_current_locale_map = &Map_US;
        }
    }
...
static auto SDL_ScancodeToAppleIdx(SDL_Scancode scancode) -> uint8_t {
    switch (scancode) {
        case SDL_SCANCODE_A: return KEYB_IDX_A;
        case SDL_SCANCODE_B: return KEYB_IDX_B;
        case SDL_SCANCODE_C: return KEYB_IDX_C;
        case SDL_SCANCODE_D: return KEYB_IDX_D;
        case SDL_SCANCODE_E: return KEYB_IDX_E;
        case SDL_SCANCODE_F: return KEYB_IDX_F;
        case SDL_SCANCODE_G: return KEYB_IDX_G;
        case SDL_SCANCODE_H: return KEYB_IDX_H;
        case SDL_SCANCODE_I: return KEYB_IDX_I;
        case SDL_SCANCODE_J: return KEYB_IDX_J;
        case SDL_SCANCODE_K: return KEYB_IDX_K;
        case SDL_SCANCODE_L: return KEYB_IDX_L;
        case SDL_SCANCODE_M: return KEYB_IDX_M;
        case SDL_SCANCODE_N: return KEYB_IDX_N;
        case SDL_SCANCODE_O: return KEYB_IDX_O;
        case SDL_SCANCODE_P: return KEYB_IDX_P;
        case SDL_SCANCODE_Q: return KEYB_IDX_Q;
        case SDL_SCANCODE_R: return KEYB_IDX_R;
        case SDL_SCANCODE_S: return KEYB_IDX_S;
        case SDL_SCANCODE_T: return KEYB_IDX_T;
        case SDL_SCANCODE_U: return KEYB_IDX_U;
        case SDL_SCANCODE_V: return KEYB_IDX_V;
        case SDL_SCANCODE_W: return KEYB_IDX_W;
        case SDL_SCANCODE_X: return KEYB_IDX_X;
        case SDL_SCANCODE_Y: return KEYB_IDX_Y;
        case SDL_SCANCODE_Z: return KEYB_IDX_Z;
        case SDL_SCANCODE_1: return KEYB_IDX_1;
        case SDL_SCANCODE_2: return KEYB_IDX_2;
        case SDL_SCANCODE_3: return KEYB_IDX_3;
        case SDL_SCANCODE_4: return KEYB_IDX_4;
        case SDL_SCANCODE_5: return KEYB_IDX_5;
        case SDL_SCANCODE_6: return KEYB_IDX_6;
        case SDL_SCANCODE_7: return KEYB_IDX_7;
        case SDL_SCANCODE_8: return KEYB_IDX_8;
        case SDL_SCANCODE_9: return KEYB_IDX_9;
        case SDL_SCANCODE_0: return KEYB_IDX_0;
        case SDL_SCANCODE_RETURN: return KEYB_IDX_RETURN;
        case SDL_SCANCODE_ESCAPE: return KEYB_IDX_ESCAPE;
        case SDL_SCANCODE_BACKSPACE: return KEYB_IDX_BACKSPACE;
        case SDL_SCANCODE_TAB: return KEYB_IDX_TAB;
        case SDL_SCANCODE_SPACE: return KEYB_IDX_SPACE;
        case SDL_SCANCODE_MINUS: return KEYB_IDX_MINUS;
        case SDL_SCANCODE_EQUALS: return KEYB_IDX_EQUALS;
        case SDL_SCANCODE_LEFTBRACKET: return KEYB_IDX_LEFTBRACKET;
        case SDL_SCANCODE_RIGHTBRACKET: return KEYB_IDX_RIGHTBRACKET;
        case SDL_SCANCODE_BACKSLASH: return KEYB_IDX_BACKSLASH;
        case SDL_SCANCODE_SEMICOLON: return KEYB_IDX_SEMICOLON;
        case SDL_SCANCODE_APOSTROPHE: return KEYB_IDX_APOSTROPHE;
        case SDL_SCANCODE_GRAVE: return KEYB_IDX_GRAVE;
        case SDL_SCANCODE_COMMA: return KEYB_IDX_COMMA;
        case SDL_SCANCODE_PERIOD: return KEYB_IDX_PERIOD;
        case SDL_SCANCODE_SLASH: return KEYB_IDX_SLASH;
        case SDL_SCANCODE_UP: return KEYB_IDX_UP;
        case SDL_SCANCODE_DOWN: return KEYB_IDX_DOWN;
        case SDL_SCANCODE_LEFT: return KEYB_IDX_LEFT;
        case SDL_SCANCODE_RIGHT: return KEYB_IDX_RIGHT;
        default: return KEYB_IDX_UNKNOWN;
    }
}
...
auto Frontend_TranslateKey(SDL_Keycode key, SDL_Keymod mod) -> uint8_t {
  ...
  if (key >= 'a' && key <= 'z') {
    ...
    switch (key) {
      case SDLK_RETURN:    apple_code = kA2Cr;    break;
      case SDLK_ESCAPE:    apple_code = kA2Esc;   break;
      case SDLK_BACKSPACE: apple_code = kA2Bs;    break;
      case SDLK_TAB:       apple_code = kA2Tab;   break;
      case SDLK_SPACE:     apple_code = kA2Space; break;
      case SDLK_LEFT:      apple_code = kA2Bs;    break;
      case SDLK_RIGHT:     apple_code = kA2Right; break;
      case SDLK_UP:        apple_code = IS_APPLE2() ? kA2Cr      : kA2UpIIe;   break;
      case SDLK_DOWN:      apple_code = IS_APPLE2() ? '/'        : kA2DownIIe; break;
      case SDLK_DELETE:    apple_code = IS_APPLE2() ? uint8_t{0} : kA2Del;     break;
...
// NOLINTNEXTLINE(bugprone-easily-swappable-parameters): scancode/keycode/mod are semantically distinct
void Frontend_DispatchKeyEvent(uint32_t scancode, uint32_t keycode, uint32_t mod, bool bDown) {
    ...
    static_assert(KEYB_IDX_UP < KEYB_MAP_SIZE, "KeyboardIdx enum value exceeds map array size");

    if (g_keyboard_mapping_mode == 1) { // Positional
        ...
        if (idx != KEYB_IDX_UNKNOWN && idx < KEYB_MAP_SIZE) {
            ...
            apple_ascii = g_current_locale_map->map[idx];
        }
    } else { // Symbolic
...
// The cast is well-defined for the enum's underlying int type; the analyzer check is overly strict.
// NOLINTNEXTLINE(clang-analyzer-optin.core.EnumCastOutOfRange)
static auto AsLinAppleKey(int v) noexcept -> LinAppleKey { return static_cast<LinAppleKey>(v); }
...
auto Frontend_ToCoreKey(int key, uint32_t mod) -> LinAppleKey {
  ...
  if (key < kAscii7BitBound && key >= kAsciiPrintableMin) {
      if (bCtrl && key >= 'a' && key <= 'z') return AsLinAppleKey(key - 'a' + 1);
      if (bShift) {
          if (key >= 'a' && key <= 'z') return AsLinAppleKey(key - 'a' + 'A');
          if (key >= '0' && key <= '9') {
            static const std::array<uint8_t, 10> shift_nums = {{')', '!', '@', '#', '$', '%', '^', '&', '*', '('}};
            return AsLinAppleKey(shift_nums.at(static_cast<size_t>(key - '0')));
          }
          switch(key) {
              case SDLK_GRAVE:        return AsLinAppleKey('~');
              case SDLK_MINUS:        return AsLinAppleKey('-');
              case SDLK_EQUALS:       return AsLinAppleKey('=');
              case SDLK_LEFTBRACKET:  return AsLinAppleKey('[');
              case SDLK_RIGHTBRACKET: return AsLinAppleKey(']');
              case SDLK_BACKSLASH:    return AsLinAppleKey('\\');
              case SDLK_SEMICOLON:    return AsLinAppleKey(';');
              case SDLK_APOSTROPHE:   return AsLinAppleKey('\'');
              case SDLK_COMMA:        return AsLinAppleKey(',');
              case SDLK_PERIOD:       return AsLinAppleKey('.');
              case SDLK_SLASH:        return AsLinAppleKey('/');
              default:                break;
          }
      }
...
auto Frontend_HandleKeyEvent(SDL_Keycode key, bool bDown) -> bool {
  switch (key) {
    ...
    case SDLK_LSHIFT:
    case SDLK_RSHIFT: {
      // Sync modifier state so the peripheral sees ctrl_key before Reset arrives.
